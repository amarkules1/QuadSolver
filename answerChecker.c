...
//returns 0 if x is accurate, -1 if 0.99999x is more accurate, and 1 if 1.00001x is more accurate to indicate that the solution may be inaccurate

int checkAnswer(double a, double b, double c, double x){
	double val = fabs((a * x * x) + (b * x) + c);
	double xSmall = 0.99999 * x;
